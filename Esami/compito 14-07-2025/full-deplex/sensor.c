...
void send_data(void* arg){
    ...
    while(1){
        ...
        if(td->value >= 75){
            printf("[Sensore %d] Entro in stato di allarme, value: %d\n", td->id, td->value);
            printf("[Sensore %d] Notifico il nodo centrale\n", td->id);
            td->status = 1;
            ...
            printf("[Sensore %d] Invio dati al nodo central, value: %d\n", td->id, td->value);
            if(send(sockfd, td, sizeof(sensor_data), 0) < 0){
                perror("send");
                exit(EXIT_FAILURE);
            }
...
int main(int argc, char* argv[]){
    ...
    printf("[Sensore %s] connesso al nodo centrale in %s:%d\n", argv[2], argv[1], PORT);
