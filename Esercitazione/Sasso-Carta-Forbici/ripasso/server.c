...
void handle_game(void* arg){
    ...
    int sockfdp1 = td->sockfd_p1, sockfdp2 = td->sockfd_p2;
    ...
    if (counter1 > counter2)
        snprintf(buffer, sizeof(buffer), "Partita terminata: Vince %s", p1->email);
    else if (counter2 > counter1)
        snprintf(buffer, sizeof(buffer), "Partita terminata: Vince %s", p2->email);

    send(sockfdp1, buffer, strlen(buffer), 0);
    send(sockfdp2, buffer, strlen(buffer), 0);
...
void handle_data(void* arg){
    ...
    while(1){
        ...
        if(!strcmp(data.operation, "registrazione")){
            ...
            if(!found && players.counter < MAX_PLAYERS){
                ...
                printf("[SERVER] Nuovo utente %s registrato\n", data.email);
            ...
            if(found){
                ...
                snprintf(buffer, sizeof(buffer), "Login Fallito, email o password non valide");
            }
            ...
            printf("[SERVER] Richiesta di matchmaking da parte di %s\n", data.email);
            ...
            if(found){
                ...
                int game_port = ntohs(game_addr.sin_port);
                
                snprintf(buffer, sizeof(buffer), "MATCH_FOUND %d", game_port);
                ...
                if(sockfdp1 < 0 || sockfdp2 < 0){
                    ...
                    if(sockfdp2 >= 0) close(sockfdp2);
                    pthread_mutex_lock(&players.lock);
                    td->status = 1; found->status = 1;
                    pthread_mutex_unlock(&players.lock);
                    return;
                }

                game_info* game = malloc(sizeof(game_info));
                game->sockfd_p1 = sockfdp1;
                game->sockfd_p2 = sockfdp2;
                game->p1 = td;
                game->p2 = found;

                pthread_t game_thread;
                if(pthread_create(&game_thread, NULL, (void*)handle_game, game) != 0){
...
int main(){
    ...
    while(1){
        ...
        if(pthread_create(&tid, NULL, (void*)handle_data, player) != 0){
            perror("pthread_create");
            close(player->sockfd);
            free(player);            
        }
